...
\newcommand{\CurrentVersion}{0.0.2} % ultima versione, da cambiare ad ogni push significativo
...
\hline
\CurrentVersion & 29/12/2025 & Aggiunta Test di Sistema e di Accettazione & Marco Piro & - & - \\
\hline
...
\MetricTable{
MPC01 & Estimated at completion (EAC) & $\pm 5\%$ rispetto al (BAC) & Budget at completion (BAC) \\
\hline
MPC02 & Estimate to complete (ETC) & $\geq 0$ & $\leq$ EAC \\
\hline
MPC03 & Actual cost (AC) & $\geq 0$ & $\leq$ EAC \\
\hline
MPC04 & Earned value (EV) & $\geq 0$ & $\leq$ EAC \\
\hline
MPC05 & Planned value (PV) & $\geq 0$ & $\leq$ Budget at completion (BAC) \\
\hline
MPC06 & Schedule variance (SV) & $\geq -5\%$ rispetto al (BAC) & $\geq 0\%$ \\
\hline
MPC07 & Cost variance (CV) & $\geq -5\%$ rispetto al (BAC) & $\geq 0\%$ \\
\hline
...
\begin{longtable}{|p{2cm}|p{8cm}|p{3cm}|p{2.5cm}|}
  \hline
  \textbf{Codice} & \textbf{Descrizione Scenario} & \textbf{Caso d'Uso} & \textbf{Esito} \\
  \hline
  \endhead
  
  TA-001 & \textbf{Registrazione e Accesso:} L'utente si registra, conferma l'account ed effettua il primo accesso con successo. & UC-0A, UC-0B & In Attesa \\
  \hline
  TA-002 & \textbf{Gestione Profilo:} L'utente accede al profilo, modifica la propria password o dati anagrafici e salva le modifiche. & UC-0D & In Attesa \\
  \hline
  TA-003 & \textbf{Ciclo completo AI Assistant:} L'utente genera un testo, lo modifica, ne cambia l'immagine e lo pubblica/salva. & UC-1 & In Attesa \\
  \hline
  TA-004 & \textbf{Gestione Storico Prompt:} L'utente cerca un vecchio prompt nello storico, lo duplica e rigenera un nuovo contenuto. & UC-1B, UC-1F & In Attesa \\
  \hline
  TA-005 & \textbf{Analisi Documentale Co-Pilot:} L'operatore carica un cedolino, verifica che l'AI abbia riconosciuto l'azienda e il dipendente, e corregge eventuali errori. & UC-2A, UC-2C & In Attesa \\
  \hline
  TA-006 & \textbf{Invio Comunicazioni:} L'operatore seleziona un documento validato, genera un messaggio da template e invia il tutto. & UC-2F, UC-2H & In Attesa \\
  \hline
  TA-007 & \textbf{Monitoraggio Analytics:} L'auditor accede alla dashboard, filtra per l'ultimo mese e visualizza i grafici di utilizzo e performance. & UC-3A & In Attesa \\
  \hline
