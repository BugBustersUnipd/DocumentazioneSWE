...
\newcommand{\CurrentVersion}{0.1.5} % ultima versione, da cambiare ad ogni push significativo
...
\usepackage{amsmath} %per scrivere formule (sezione qualità)
...
\textbf{Data ultima modifica}     & 14/1/2026 \\
\\
\end{tabularx}
\end{tcolorbox}
\end{center}
...
\hline
\rowcolor{secondaryblue!10} \CurrentVersion & 18/1/2026 & Aggiornamento sezione qualità, aggiunti dettagli su metriche di processo & Linor Sadè & - & - \\
\hline
\rowcolor{secondaryblue!10} 0.1.4 & 13/1/2026 & continuo sezione qualità, aggiunti: standard e modelli, principi di qualità del processo, metriche di processo  & Linor Sadè & - & - \\
\hline
\rowcolor{secondaryblue!10} 0.1.3 & 12/1/2026 & Inizio sezione qualità, modificato inizio precedente& Linor Sadè & - & - \\
\hline
\rowcolor{secondaryblue!10} 0.1.2 & 4/1/2026 & Aggiunte parti mancanti relative al piano di qualifica. Ulteriori correzioni al contenuto precedente. & Linor Sadè & - & - \\
\hline
...
\hline
\rowcolor{secondaryblue!10} 0.0.10 & 22/12/2025 & Aggiunto Piano di Progetto e di Qualifica & Marco Piro & - & - \\
\hline
\rowcolor{secondaryblue!10} 0.0.9 & 18/12/2025 & Eliminato i Processi di acquisizione ed aggiunto le lettere di Candidatura e Presentazione & Marco Piro & - & - \\
\hline
...
Il nostro progetto mira alla creazione di un'applicazione stand-alone (rispetto l'applicazione Nexum già esistente) che implementa un modulo di AI assistant generativo per la scrittura di comunicazioni e l'AI Co-Pilot\G{} per
...
Per ulteriori informazioni sul progetto si rimanda all'analisi dei requisiti al link (AGGIUNGERE LINK). %TODO
...
Per maggiore usabilità e facilitá di consultazione, il glossario è accessibile anche aprendo dal nostro sito web i vari documenti con il viewer
...
tale processo ha l'obiettivo di chiarire i requisiti richiesti dalla proponente, identificare vincoli e risorse, 
...
\begin{itemize}
    ...
    capitolato d'appalto. In questa fase si pongono domande alla proponente per chiarire eventuali dubbi o ambiguità, processo utile alla scelta finale.
    \item \textbf{Stima delle risorse}: definizione delle tempistiche, dei costi totali e delle risorse necessarie per la 
    realizzazione del progetto.
    \item \textbf{Analisi dei requisiti e contrattazione con la proponente}: identificazione e documentazione dei requisiti 
    ...
    \item \textbf{Pianificazione del progetto}: suddivisione del progetto in fasi, definizione delle milestone aggiuntive (oltre a RTB e PB), 
    ...
    l'Analisi dei requisiti, Norme di Progetto e altri documenti di supporto. La documentazione prodotta sarà utilizzata sia come 
    strumento di lavoro interno al team sia come riferimento e strumento di controllo da parte della proponente.
    \item \textbf{Comunicazione con la proponente}: stabilire canali di comunicazione efficaci per garantire un flusso informativo 
    ...
    \item \textbf{Revisione e approvazione}: sottoporre tutta la documentazione prodotta alla revisione e approvazione della proponente e del committente, 
...
\begin{itemize}
    ...
    \item \textbf{GitLab}: Per il codice già esistente fornito dalla proponente, utile all'implementazione di un prodotto che possa essere facilmente integrabile alla piattaforma già esistente.
    ...
    \item \textbf{Telegram e Gmail}: Per la comunicazione con la proponente e per inviare documenti ufficiali. 
\end{itemize}

\subsubsection{Comunicazione e Organizzazione}
Le comunicazioni con la proponente avvengono solitamente a cadenza bisettimanale mercoledì alle 15:00, salvo imprevisti o esigenze particolari. 
I verbali relativi agli incontri con la proponente 
vengono condivisi e archiviati nel repository della 
documentazione per garantire trasparenza e reperibilità. 
Tutte le decisioni rilevanti emerse negli incontri con la proponente devono essere formalmente riportate e documentate 
con il sistema di ticketing su GitHub e nella documentazione ufficiale. Eventuali problemi 
critici o dubbi vengono comunicati immediatamente 
sui canali concordati con la proponente, in modo da garantirne la 
...
\begin{itemize}
    ...
    \item \href{http:\\localhost}{Glossario}
    \item \href{http:\\localhost}{Analisi dei requisiti}
    \item \href{http:\\localhost}{Piano di progetto}
    \item \href{http:\\localhost}{Piano di qualifica}
\end{itemize}
...
garantire una comunicazione chiara e univoca tra tutti i membri del team, con la proponente, docenti (il committente) e con i lettori esterni.
Per una consultazione rapida durante la visualizzazione dei documenti, il glossario è accessibile anche tramite il
viewer PDF sviluppato dal team che si trova nella repository web ufficiale della documentazione accessibile al seguente \href{https://bugbustersunipd.github.io/DocumentazioneSWE/}{link}.
...
per la realizzazione del progetto Nexum, impegnandosi a rispettare tali condizioni durante l'intero ciclo di vita del progetto.
...
La Lettera di Candidatura rappresenta il documento formale con cui il gruppo BugBusters ha ufficializzato la propria candidatura
per il capitolato proposto dall'azienda Eggon. 
Al suo interno sono sintetizzate le motivazioni che hanno determinato la scelta di questo specifico progetto, 
evidenziando i punti di interesse tecnologico e formativo, oltre a fornire i riferimenti al documento di dichiarazione degli impegni e valutazione dei capitolati\G{}.
\begin{center}
    \begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{3.2cm}|>{\centering\arraybackslash}X|}
    \hline
    \rowcolor{primaryblue!40}
...
\begin{itemize}
    \item Lettera di Presentazione per la \textbf{Requirements and Technology Baseline (RTB)}
    \item Lettera di Presentazione per la \textbf{Product Baseline (PB)}
\end{itemize}
\begin{center}
\begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{3.2cm}|>{\centering\arraybackslash}X|}
\hline
...
azioni concordate durante le riunioni interne del team e le riunioni con la proponente Eggon.
Si dividono in verbali interni, prodotti durante le riunioni del team, e verbali esterni, 
redatti dopo gli incontri con la proponente.
...
Il Piano di Progetto rappresenta il documento di riferimento per la pianificazione strategica e operativa delle attività del gruppo BugBusters. 
Esso definisce la scansione temporale del lavoro suddiviso in sprint, l'allocazione delle risorse umane e strumentali, nonché l'analisi preventiva dei rischi con le relative strategie di mitigazione. 
Il documento ha inoltre la funzione fondamentale di monitorare l'andamento del progetto, riportando a ogni avanzamento il consuntivo delle ore produttive e dei costi sostenuti rispetto a quanto preventivato, 
...
Il Piano di Qualifica documenta la strategia adottata dal gruppo BugBusters per garantire la qualità del prodotto software che il gruppo deve realizzare e dei processi correlati. 
Esso definisce nel dettaglio le metodologie di verifica e validazione, gli standard di qualità di riferimento e le metriche utilizzate per il monitoraggio. 
Il documento riporta inoltre la pianificazione e gli esiti dei test effettuati, con l'obiettivo specifico di assicurare il rispetto dei requisiti e mantenere una copertura del codice (code coverage) pari o superiore all'80\%.
\begin{center}
    \begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{3.2cm}|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|}
    \hline
    \rowcolor{primaryblue!40}
...
Il processo di sviluppo definisce le attività tecniche e operative necessarie per la realizzazione del prodotto software che il gruppo deve realizzare,
in conformità ai requisiti stabiliti durante la fase di fornitura. Questo processo include la progettazione, 
l'implementazione, il testing e la documentazione del software, garantendo che ogni fase sia eseguita secondo
standard di qualità elevati e best practice del settore.

\subsubsection{Attività previste}
\begin{itemize}
    \item \textbf{Implementazione del processo}: se non definito prima, il fornitore dovrebbe definire o scegliere un modello di ciclo di vita del software appropriato alla complessità del progetto;
    \item \textbf{Analisi dei requisiti di sistema}: l'uso previsto del sistema da sviluppare deve essere analizzato per definire i requisiti di sistema. La specifica dei requisiti di sistema deve descrivere le funzioni e le capacità del sistema, i requisiti di business, organizzativi e degli utenti, nonché i requisiti relativi a sicurezza, protezione, fattori umani (ergonomia), interfacce, operatività e manutenzione. Devono inoltre essere inclusi i vincoli di progettazione e i requisiti di qualificazione;
    ...
    \item \textbf{Analisi dei requisiti del software}: il fornitore deve stabilire e documentare i requisiti del software, includendo caratteristiche di qualità: come le caratteristiche funzionali, i requisiti di sicurezza e di interfaccia esterna del software;
    ...
    \item \textbf{Scrittura e test del codice}: il fornitore deve scrivere il codice per tutte le componenti individuate, e testarle esaustivamente;
    \item \textbf{Integrazione del software}: il fornitore deve sviluppare un piano di integrazione per assemblare le unità e i componenti software nel prodotto software finale. Il piano deve includere i requisiti di test, le procedure, i dati necessari, le responsabilità e la pianificazione delle attività;
    \item \textbf{Test di qualifica del software}: il fornitore deve eseguire i test di qualificazione in conformità ai requisiti di qualificazione del software. Deve essere verificato che l’implementazione di ogni requisito software sia testata per garantirne la conformità;
    \item \textbf{Integrazione del sistema}: gli elementi di configurazione software devono essere integrati con quelli hardware, con le operazioni manuali e con altri sistemi, se necessario, all’interno del sistema complessivo. Gli insiemi risultanti devono essere testati progressivamente rispetto ai requisiti previsti;
    ...
    \item \textbf{Supporto per l'approvazione del software}: il fornitore deve supportare il committente nella revisione e nei test di accettazione del prodotto software. Le attività di accettazione devono tenere conto dei risultati delle revisioni congiunte, degli audit, dei test di qualificazione del software e, se eseguiti, dei test di qualificazione del sistema; 
\end{itemize}
\subsubsection{Analisi Dei Requisiti\G}
È un attività fondamentale della Requirements and Technology Baseline (RTB) e consiste nella raccolta, analisi e documentazione di tutti i requisiti che il sistema sviluppato dovrà soddisfare.
Tale attività è documentata nell'apposito documento di Analisi dei Requisiti(TODO inserire link), che servirà poi come riferimento per guidare tutte le fasi successive di progettazione, implementazione e testing del software.
Nella sezione seguente viene illustrato il sistema di nomenclatura adottato per identificare in modo univoco i casi d'uso.
...
dove:
\begin{itemize}
    \item \textbf{UC}: sta per Use Case (Caso d'Uso);
    \item \textbf{SezionePrincipale}: abbiamo identificato quattro moduli principali identificati da un numero da 0 a 3;
    \item \textbf{Principale}: è identificato da una lettera dell'alfabeto, e rappresenta un caso d'uso primario non correlato ad altri casi d'uso, se non tramite inclusione;
...
dove:
\begin{itemize}
    \item \textbf{Tipologia}: rappresenta la categoria del requisito, e può essere funzionale (RF), requisiti di qualità (RQ), requisiti di vincolo (RV) o requisiti prestazionali (RP);
    \item \textbf{Numero}: è un numero progressivo che identifica in modo univoco il requisito all'interno della sua tipologia.
\end{itemize}
...
facilitando la comunicazione tra i membri del team, la proponente e gli stakeholder esterni. Una documentazione ben strutturata supporta
...
\begin{itemize}
    ...
    Vengono utilizzate le funzionalità di issue tracking e project board per tracciare le attività di documentazione. 
    ...
    \item \textbf{LaTeX}: Per la stesura di documenti tecnici e formali, garantendo un formato professionale e coerente. È stata stabilita
    un'identità visiva comune per tutti i documenti, inclusi template, stili e convenzioni di formattazione.
...
\begin{enumerate}
    \item \textbf{Norme di Progetto}: Il presente documento, dove vengono definite linee guida metodologiche, standard di lavoro e convenzioni adottate dal team.
    \item \textbf{Piano di Progetto}: Documento dettagliato con allocazione delle risorse umane e materiali per le milestone e le consegne.
    ...
    \item \textbf{Verbali (interni/esterni)}: Registrazione degli incontri e delle decisioni, sia per le riunioni interne sia per i confronti con la proponente.
\end{enumerate}
...
Dal momento che il ruolo del responsabile costituisce il maggior dispendio di budget all'interno del progetto e che la redazione dei verbali è una delle sue principali responsabilità,
è stato deciso di adottare una struttura standardizzata per i verbali al fine di ottimizzare il tempo impiegato nella loro redazione, introducendo anche delle macro per LaTeX che permettono di velocizzare ulteriormente la creazione dei verbali.
...
discussione e condivisione delle difficoltà e dubbi incontrati durante lo sviluppo del progetto. 
...
\begin{itemize}
    ...
    \item \textbf{Revisione}: Una volta completata la bozza, il documento viene sottoposto al verificatore designato che controlla
     la correttezza, la coerenza e la conformità agli standard di qualità. Il processo è asincrono e 
     avviene tramite pull request su GitHub. Se ci sono modifiche vengono tracciate e discusse all'interno della pull request. 
     La verifica verrà tracciata all'interno del registro delle modifiche del documento.
...
\begin{itemize}
    \item \textbf{Board}: Serve per definire lo stato in cui si trova una issue\G{}, ovvero se: è ancora da iniziare, è in sviluppo o è terminata. Il team ha scelto il modello Kanban per la gestione delle issue.
    ...
    Il team BugBusters, oltre che per un corretto workflow di verifica, usa le pull request come unico modo di modifica in produzione, in quanto si vuole che il branch main sia modificato solo a seguito di una pull request con 2 review positive.
\end{itemize}
...
Il processo di Verifica\G{} ha come obiettivo principale quello di verificare che quanto prodotto sia a regola d'arte, ovvero conforme con i requisiti richiesti.\\
L'obiettivo della verifica\G{} è poter rispondere alla domanda <<Did I build the system right?>> gli esiti di tale processo sono riportati nel documento di Piano di Qualifica nella sezione dedicata ai test.\\
Notare che al momento della condivisione di questo documento al pubblico, il team BugBusters avrà solamente raggiunto l'RTB, per questo motivo lo stato dei test sara "Non Implementato". Al raggiungimento della PB, questa sezione verrà aggiornata con i risultati dei test eseguiti.
\subsubsubsection{Attività di verifica}
La verifica agisce su singoli segmenti di sviluppo, accertando che l'esecuzione di essi non abbia introdotto errori. 
In questa prospettiva, un'attività di verifica è il controllo che il prodotto ottenuto al termine di una fase sia congruente con il semilavorato avuto come punto di partenza di quella fase.\\
...
In ogni caso tutte le informazioni relative alla verifica, verranno riportate nel Piano di Qualifica (INSERIRE LINK FUTURO E SEZIONE CORRISPONENTE). %TODO
...
\begin{itemize}
    ...
    \item \textbf{Automatizzabili}: poiché i test possono essere centinaia, è necessario automatizzarli mediante  driver (che serve per pilotare il test), stub (che simula i moduli necessari al test\G{} ma che non ne sono oggetto) e logger (che registra ciò che avviene durante l'esecuzione).
\end{itemize}
...
\begin{itemize}
    \item \textbf{Codice}: un codice univoco che garantisce la traccibilità del test (quindi la sua identificazione univoca);
    \item \textbf{Descrizione}: una breve descrizione dell'obiettivo del test;
    \item \textbf{Requisito}: codice univoco del requisito a cui il test fa riferimento. Tale codice è reperibile nell'analisi dei requisiti nella sezione 3.1 Requisiti funzionali (INSERIRE LINK ANALISI DEI REQUISITI E IN CASO AGGIORNARE NOME); %TODO
    \item \textbf{Stato}: lo stato del test, che può essere:
    \begin{itemize}
        \item \textbf{Non Implementato}: il test non è ancora stato implementato;
        \item \textbf{Superato}: il test è stato implementato ed eseguito;
        \item \textbf{Non Superato}: il test è stato implementato ed eseguito, ma non ha raggiunto i criteri di accettazione;
        \item \textbf{Da Eseguire}: il test è stato implementato, ma non è ancora stato eseguito.
    \end{itemize}
\end{itemize}
...
%TODO
Si fa notare che al momento della condivisione di questo documento al pubblico, il team BugBusters avrà solamente raggiunto l'RTB, per questo motivo lo stato dei test sara "In Attesa". Al raggiungimento della PB, questa sezione verrà aggiornata con i risultati dei test. (MODIFICARE EVENTUALMENTE VALORE DI STATO/ESITO) 
...
gestire le risorse e comunicare con la proponente e i docenti.
...
\begin{itemize}
    ...
    \item gestire le richieste di chiarimento e modifica dei requisiti con la proponente;
...
garantire che il codice sia di alta qualità, efficiente e mantenibile.
In particolare dovrà occuparsi di:
\begin{itemize}
...
\begin{itemize}
    ...
    garantendo tracciabilità e storicizzazione delle decisioni. Utili in quanto permettono di comunicare solo con i membri interessati alla discussione.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Telegram}: utilizzato per comunicazioni rapide e coordinamento con la proponente.
\end{itemize}
...
Questi incontri avvengono tramite piattaforma Google Meet.
Al termine di ogni riunione, viene redatto un verbale che riassume le decisioni prese e le azioni da intraprendere, il quale verrà condiviso con la proponente e, se in linea con quanto discusso, firmato.
\section{Qualità del Software}

% Elenco puntato da eliminare e trasformare in testo fluido e privo di parolacce:
% \begin{itemize}
%     \item introduzione, spiegare brevemente: che cos'è la qualità del software, su cosa si misura la qualità: prodotto e processo, perchè processo influenza il prodottoo, il processo di valutazione cosa fa
%     \item spiegare il processo di valutazione: definizione, selezione delle metriche e misurazione
%     \item caratteristiche della qualità di processo.
% \end{itemize}
Per qualità\G{} si intende l'insieme delle caratteristiche di un'entità che ne determinano la capacità di soddisfare esigenze sia espresse che implicite.La qualità si misura su due aspetti principali: 
\begin{enumerate}
    \item la \textbf{qualità del prodotto};
    \item la \textbf{qualità del processo}.
\end{enumerate}
\textbf{La qualità del prodotto finale è una conseguenza diretta della qualità del processo utilizzato per crearlo.}
Perché il software risulti di qualità è necessario che venga pianificato, e non solo controllato, un buon processo di valutazione: questo permette di lavorare seguendo una modalità proattiva (valutazione continua) piuttosto che reattiva (correzione di bug).\\
Diventa quindi fondamentale seguire uno standard di riferimento per poter definire, misurare e valutare la qualità del software in modo oggettivo e coerente.
...
\begin{itemize}
    \item Qualità del Prodotto, che analizza le seguenti proprietà intrinseche del software:
    \begin{enumerate}
    ...
    \end{enumerate}
    \item Qualità in Uso, che valuta le seguenti caratteristiche dal punto di vista dell'utente finale:
    \begin{enumerate}
        \item \textbf{Efficacia};
        \item \textbf{Efficienza};
        \item \textbf{Soddisfazione};
        \item \textbf{Libertà da rischi};
        \item \textbf{Contesto di copertura}.
    \end{enumerate}
\end{itemize}
Noi ci concentriamo sulla qualità del prodotto. \\ %qui forse è da modificare, perchè noi non abbiamo metriche di qualità in uso? solo del prodotto e del processo.
Per quanto riguarda, invece, la qualità del processo, lo standard di riferimento è l'\textbf{ISO/IEC 9001} e il \textbf{modello CMMI (Capability Maturity Model Integration)}. Questi standard forniscono linee guida per la gestione della qualità nei processi di sviluppo software, includendo aspetti come la pianificazione, il controllo e il miglioramento continuo dei processi stessi.
...
\begin{enumerate}
    \item \textbf{Fase preventiva di definizione di metriche, interpretazione delle misure e criteri di accettazione}: sia nel contesto del prodotto che del processo, vengono identificate le metriche e come queste devono essere interpretate per valutare la qualità e raggiungere l'obiettivo di accettazione.
    \item \textbf{Misurazione}: la raccolta dei dati necessari per calcolare le metriche selezionate. Questa fase dovrebbe essere automatizzata attraverso strumenti di analisi statica e dinamica del codice, test automatici e monitoraggio delle prestazioni.
    \item \textbf{Valutazione}: l'analisi dei dati raccolti per determinare se gli obiettivi di qualità sono stati raggiunti. Questa fase può includere la revisione dei risultati da parte di un team di verifica e validazione, nonché la documentazione dei risultati e delle eventuali azioni correttive da intraprendere.
    \item \textbf{Accettazione}: la decisione finale su se il software soddisfa i requisiti di qualità stabiliti. Se le metriche sono soddisfatte, il software può essere considerato pronto.
\end{enumerate}
Questo intero processo è documentato all'interno del \href{https://bugbustersunipd.github.io/DocumentazioneSWE/RTB/PIANO%20DI%20QUALIFICA/Piano_di_Qualifica.pdf}{Piano di Qualifica}.
\subsection{Principi della Qualità del processo}
Secondo lo standard ISO/IEC 9001, la qualità del processo di sviluppo software si basa su diversi principi fondamentali:
\begin{itemize}
    \item \textbf{Responsabilità della direzione}: il responsabile deve definire una politica per la qualità, stabilire obiettivi e garantire che tutti i membri del team siano consapevoli delle loro responsabilità.
    ...
    \item \textbf{Realizzazione del prodotto}: il processo di sviluppo richiede che le fasi di documentazione dei requisiti, progettazione e sviluppo siano sottocontrollo e tracciabili.
...
Ciò viene fatto in questo progetto attraverso una chiara definizione degli strumenti e delle metodologie di lavoro (specificate in questo documento), un monitoraggio continuo dell'avanzamento del progetto (tramite issue tracking e riunioni periodiche documentate), un'attenta analisi sulle richieste della proponente (specificate nel documento di \href{https://bugbustersunipd.github.io/DocumentazioneSWE/RTB/ANALISI%20DEI%20REQUISITI/analisi_dei_requisiti_.pdf}{analisi dei requisiti})  e una valutazione regolare della qualità del prodotto con un'analisi sul miglioramento (tramite il processo di verifica e validazione descritto nel \href{https://bugbustersunipd.github.io/DocumentazioneSWE/RTB/PIANO%20DI%20QUALIFICA/Piano_di_Qualifica.pdf}{piano di qualifica}).

\subsection{Metriche per la qualità}
%TODO, magari inserire una descrizione di cosa sono le metriche (accennate nella sezione del processo di valutazione della qualità)
...
Le metriche selezionate per questo progetto vengono definite qui e misurate durante il processo di valutazione della qualità descritto nel \href{https://bugbustersunipd.github.io/DocumentazioneSWE/RTB/PIANO%20DI%20QUALIFICA/Piano_di_Qualifica.pdf}{Piano di Qualifica}.\\
Le metriche sono suddivise in due categorie principali:
\begin{enumerate}
    \item \textbf{Metriche per la qualità del prodotto}: queste metriche valutano aspetti come la funzionalità, l'affidabilità, l'efficienza, l'usabilità, la manutenibilità e la portabilità del software.
    \item \textbf{Metriche per la qualità del processo}: queste metriche monitorano l'efficacia del processo di sviluppo, inclusi tempi di consegna, tassi di difetti, copertura dei test  e conformità agli standard di qualità.
\end{enumerate}
...
\paragraph*{Fornitura}{
    \begin{enumerate}[label=\textbf{MPC0\arabic*}, leftmargin=4.5em, series=metriche_processo] %nota: viene definita una serie per far sì che gli enumerate riprendano la numerazione da dove si era interrotta
        \item \textbf{Earned Value (EV)}: misura il valore del lavoro effettivamente completato rispetto al costo preventivato.
        \[ EV = BAC \times \text{Percentuale di completamento} \]
        dove $BAC$ (Budget at Completion) è il budget totale previsto per il progetto.

        \item \textbf{Planned Value (PV)}: misura il valore del lavoro pianificato fino a una certa data.
        ...
        \\ \textit{Valore ideale}: per ogni sprint il valore deve essere <= EAC

        \item \textbf{Actual Cost (AC)}: misura il costo effettivo sostenuto per il lavoro completato fino a una certa data (che coincide con quella di fine sprint).

        \item \textbf{Cost Performance Index (CPI)}: indica l'efficienza dei costi del progetto.
        \[ CPI = \frac{EV}{AC} \]
        ...
        \item \textbf{Schedule Performance Index (SPI)}: indica l'efficienza del tempo del progetto.
        \[ SPI = \frac{EV}{PV} \]
        ...
        \item \textbf{Estimated at Completion (EAC)}: stima il costo totale del progetto alla sua conclusione, basandosi sulle prestazioni attuali.
        \[ EAC = \frac{BAC}{CPI} \]
        \textit{Interpretazione}: Risponde alla domanda: <<Quanto costerà il progetto se le attuali prestazioni di costo continuano?>>.\\Valore ideale = BAC
        
        \item \textbf{Estimate to Complete (ETC)}: stima il costo necessario per completare il lavoro rimanente del progetto.
        \[ ETC = EAC - AC \]
        \textit{Interpretazione}: Risponde alla domanda: <<Quanto costerà completare il progetto?>>.\\Valore ideale <= EAC
        
        \item \textbf{Time Estimate At Completion (TEAC)}: stima il tempo totale necessario per completare il progetto.
        \[ TEAC = \frac{\text{Durata Pianificata}}{SPI} \]
        dove $SPI$ (Schedule Performance Index) è $\frac{EV}{PV}$.
        \textit{Interpretazione}: Risponde alla domanda: <<Quanto tempo ci vorrà per completare il progetto se le attuali prestazioni di tempo continuano?>>.\\Valore ideale <= Durata pianificata.

    \end{enumerate}
}
\paragraph*{Sviluppo}{
    \begin{enumerate}[label=\textbf{MPC0\arabic*}, leftmargin=4.5em, resume=metriche_processo]
    \item \textbf{Requirements Stability Index (RSI)}: misura la stabilità dei requisiti durante il ciclo di vita del progetto.
    \[
    RSI = \left( 1 - \frac{N_{agg} + N_{mod} + N_{canc}}{N_{iniz}} \right) \times 100
    \]
    dove:
    \begin{itemize}
        \item $N_{agg}$ è il numero di nuovi requisiti emersi dopo l’inizio del periodo;
        \item $N_{mod}$ è il numero di requisiti modificati;
        \item $N_{canc}$ è il numero di requisiti rimossi;
        \item $N_{iniz}$ è il numero di requisiti definiti all’inizio del periodo.
    \end{itemize}
    \textit{Interpretazione}: un valore più alto indica una maggiore stabilità dei requisiti.
\end{enumerate}
...
\paragraph*{Documentazione}
{
    \begin{enumerate}[label=\textbf{MPC\arabic*}, leftmargin=4.5em, resume=metriche_processo]%nota: qui fortunatamente si arriva a 10 quindi posso togliere lo 0 che avevo messo negli enumerate precedenti, se vengono cambiate le metriche allora forse questa fortuna non ci sarà più
        \item \textbf{Indice di Gulpease}: misura la leggibilità di un testo in lingua italiana, utile per valutare la documentazione tecnica.
...
\paragraph*{Verifica}{
    \begin{enumerate}[label=\textbf{MPC\arabic*}, leftmargin=4.5em, resume=metriche_processo] 
        ...
        \item \textbf{Code Coverage}: misura la percentuale di codice sorgente coperto dai test.
        \[ \text{Code Coverage} = \frac{\text{Linee di codice testate}}{\text{Linee di codice totali}} \times 100\% \]
        \textit{Interpretazione}: Un valore più alto indica una maggiore copertura dei test, riducendo il rischio di bug non rilevati.

        \item \textbf{Test Success Rate}: misura la percentuale di test superati con successo.
        \[ \text{Test Success Rate} = \frac{\text{Numero di test superati}}{\text{Numero totale di test eseguiti}} \times 100\% \]
        \textit{Interpretazione}: Un valore più alto indica una maggiore affidabilità del software.
    \end{enumerate}
}
\paragraph*{Gestione della qualità}{
    \begin{enumerate}[label=\textbf{MPC\arabic*}, leftmargin=4.5em, resume=metriche_processo]
        \item \textbf{Quality metrics satisfied}: misura la percentuale di metriche di qualità soddisfatte rispetto a quelle definite nel piano di qualifica.
        \[ \text{Quality metrics satisfied} = \frac{\text{Numero di metriche soddisfatte}}{\text{Numero totale di metriche}} \times 100\% \]
        \textit{Interpretazione}: Un valore più alto indica una maggiore conformità agli standard di qualità stabiliti.
...
\paragraph*{Gestione dei processi}{
    \begin{enumerate}[label=\textbf{MPC\arabic*}, leftmargin=4.5em, resume=metriche_processo]
        \item \textbf{Time efficiency}: misura l'efficienza nella gestione del tempo rispetto alla pianificazione.
...
\begin{enumerate}[label=\textbf{MPD0\arabic*}, leftmargin=4.5em, series=metriche_processo]
    \item \textbf{Requisiti obbligatori soddisfatti}.\\
    \textit{Valore ideale}: 100

    \item \textbf{Requisiti desiderabili soddisfatti}.\\
    \textit{Valore ideale}: 100 \textit{(accettato 0)}

    \item \textbf{Requisiti opzionali soddisfatti}.\\
    \textit{Valore ideale}: 100 \textit{(accettato 0)}

    \item \textbf{AI Acceptance Rate (rating $\geq 3/5$)}.\\
...
\begin{enumerate}[label=\textbf{MPD0\arabic*}, leftmargin=4.5em, series=metriche_processo]
    \item \textbf{UI Response Time (Interfaccia)}.\\
    \textit{Valore di accettazione}: $\leq 2$ sec\\
    \textit{Valore ideale}: $\leq 0.5$ sec

    \item \textbf{Core Response Time (AI / Upload)}.\\
...
\begin{enumerate}[label=\textbf{MPD0\arabic*}, leftmargin=4.5em, series=metriche_processo]
    \item \textbf{Click Count (Funzioni principali)}. Misura il numero di click necessari per accedere alle funzioni principali.\\
    \textit{Valore di accettazione}: $\leq 5$ click\\
    \textit{Valore ideale}: $\leq 3$ click

    \item \textbf{User Error Rate (Errori di validazione)}.\\
    \textit{Valore di accettazione}: $\leq 10\%$\\
    \textit{Valore ideale}: $\leq 5\%$
\end{enumerate}
\subsubsubsection{Manutenibilità}
\begin{enumerate}[label=\textbf{MPD0\arabic*}, leftmargin=4.5em, series=metriche_processo]
    \item \textbf{Blocker Code Smells}. Misura la presenza di "code smells" di tipo blocker, ovvero problemi nel codice che possono causare gravi malfunzionamenti o difficoltà di manutenzione.\\
    \textit{Valore di accettazione}: 0\\
    \textit{Valore ideale}: 0

    \item \textbf{Cyclomatic Complexity (per metodo)}. Misura la complessità ciclomatica di un metodo, ovvero il numero di percorsi indipendenti nel codice.\\
    \textit{Valore di accettazione}: $\leq 15$\\
    \textit{Valore ideale}: $\leq 10$

    \item \textbf{Comment Intensity}.\\
    \textit{Valore di accettazione}: $\geq 10\%$\\
    \textit{Valore ideale}: $\geq 20\%$
\end{enumerate}
\subsubsubsection{Portabilità}
\begin{enumerate}[label=\textbf{MPD0\arabic*}, leftmargin=4.5em, series=metriche_processo]
    \item \textbf{Supported Browsers (Test passati)}.\\
...
% LASCIA COMMENTATO, forse metterò...
...
% \begin{itemize}
%     \item \textbf{Appropriatezza}: definisce se il prodotto software offre le funzioni necessarie per svolgere i compiti fissati dalla proponente.
%     \item \textbf{Correttezza}: è la capacità del prodotto software di fornire risultati corretti in base alle specifiche.
%     \item \textbf{Completezza}: è la capacità del prodotto software di fornire tutte le funzioni richieste senza omissioni.
% \end{itemize}
...
% L'usabilità è la capacità del prodotto software di essere compreso e utilizzato dall'utente finale.\\ Viene misurato nello specifico:
% \begin{itemize}
...
%     \item \textbf{Conformità}: è la capacità del prodotto ad aderire a standard relativi all'usabilità.
% \end{itemize}
% \subsubsection{Manutenibilità}
% La manutenibilità è la capacità del software di essere modificato, includendo correzioni, adattamenti o miglioramenti.\\ Viene misurato nello specifico:
% \begin{itemize}
...
% La portabilità definisce la capacità del software di essere trasportato da un ambiente di lavoro ad un altro.\\ Viene misurato nello specifico:
% \begin{itemize}
