...
\begin{center}
{\large\textbf{\color{secondaryblue} Versione 0.0.5}}
...
\hline
\rowcolor{secondaryblue!10} \CurrentVersion & XXX & Approvazione del documento & - &  & XXX \\
\hline
\rowcolor{secondaryblue!10} 0.1.0 & 09/12/2025 & Verifica del documento & - & Linor Sadè & - \\
\hline
...
La nomenclatura utilizzata per evidenziare che una parola, con annessa la sua definizione, è presente all'interno del Glossario viene indicata come segue: \textbf{parola\textsubscript{\scalebox{0.6}{\textbf{G}}}}.
\newpage

\section{A}

\subsection{Abstract Class}
Classe astratta nella programmazione orientata agli oggetti che non può essere istanziata direttamente e che serve come base per altre classi. Definisce un'interfaccia comune e può contenere metodi astratti (senza implementazione) e metodi concreti (con implementazione).
...
Metrica di copertura del testing strutturale (white-box) che misura la percentuale di rami (decisioni) in un programma eseguiti da un insieme di test. Un ramo è tipicamente il risultato di una condizione (es. if-else, switch). Un coverage del 100\% significa che ogni possibile esito di ogni decisione è stato testato.
...
Si riferisce a un sistema, un'applicazione o un componente software che opera in modo autonomo e indipendente, senza richiedere l'integrazione o la dipendenza da altri sistemi esterni per funzionare. Un'applicazione stand-alone è generalmente autosufficiente, contiene al suo interno tutte le risorse necessarie per l'esecuzione e non dipende da servizi di rete, server remoti o altre infrastrutture esterne per fornire le proprie funzionalità di base.
