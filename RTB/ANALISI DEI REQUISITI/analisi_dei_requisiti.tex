...
\newcommand{\CurrentVersion}{0.0.14} % ultima versione, da cambiare ad ogni push significativo
...
\rowcolor{secondaryblue!10} \CurrentVersion & 23/12/2025 & Correzioni e aggiunte minime alla sezione 1. Migliorie grafiche & Linor Sadè & - & - \\
\hline
\rowcolor{secondaryblue!10} 0.0.13 & 12/12/2025 & Aggiunta sotto-casi d'uso alla sezione 2 & Leonardo Salviato & - & - \\
\hline
\rowcolor{secondaryblue!10} 0.0.12 & 11/12/2025 & Aggiunto modulo 3 & Marco Piro & - & - \\
\hline
\rowcolor{secondaryblue!10} 0.0.11 & 10/12/2025 & Rinominazione casi d'uso, aggiunta sotto-casi d'uso alle sezioni 0 e 1. & Leonardo Salviato & - & - \\
\hline
\rowcolor{secondaryblue!10} 0.0.10 & 07/12/2025 & Verifica dei primi casi d'uso & - & Linor Sadè & - \\
\hline
...
A seguito delle nuove decisioni progettuali rispetto alle proposte del capitolato, il Sistema non sarà inizialmente integrato nella piattaforma NEXUM, ma verrà realizzato come \textbf{applicazione standalone}, autonoma e indipendente da essa. Tale applicazione implementerà i moduli "AI Assistant Generativo" e "AI Co-Pilot per i CdL" in un ambiente isolato, così da consentire una fase di sviluppo, test e validazione più controllata. Eggon avrà la possibilità di valutare il prototipo standalone ed eventualmente procedere con l'integrazione nella piattaforma NEXUM.
...
Il prodotto che BugBusters si propone di sviluppare è una versione standalone dei moduli "AI Assistant Generativo" e "AI Co-Pilot per i CdL", svincolata dalla piattaforma NEXUM. Tale applicazione costituirà un prototipo funzionale in grado di operare autonomamente e di implementare le principali funzionalità richieste dal Committente, senza dipendere dalla piattaforma esistente.

Tale scelta deriva dalla necessità di rispettare le politiche di privacy e sicurezza dei dati della Proponente, le quali risultano incompatibili con la richiesta della Committente di rendere il prodotto accessibile al pubblico.
...
\begin{itemize}
    ...
    con possibilità di selezionare tono, stile.
...
l'obiettivo del progetto è quello di progettare moduli intelligenti e interoperabili per essere in un futuro integrati nella piattaforma NEXUM e tale ecosistema è concepito per rispondere alle esigenze di un ampio spettro di organizzazioni e professionisti nel settore delle risorse umane e della consulenza del lavoro.
...
\begin{itemize}
    ...
    \item \textbf{Sistema (AI Doc Classifier)}: Il modulo intelligente incaricato di analizzare visivamente il documento, applicare l'OCR e classificarne la tipologia (es. "Cedolino", "CUD").
    \item \textbf{Sistema (Entity Resolver)}: Il componente che analizza il testo estratto per identificare univocamente i destinatari (es. Nome, Cognome, CF) confrontandoli con l'anagrafica.
    \item \textbf{Sistema (Splitter)}: L'agente automatico che scansiona i documenti massivi (es. PDF multipagina) e li suddivide in singoli file, uno per ciascun destinatario individuato.
    \item \textbf{Sistema (Dispatcher \& Tracking)}: Il modulo responsabile della creazione dei pacchetti di invio e della generazione delle ricevute di consegna (simulata in ambiente standalone).
...
\textbf{Post-condizioni}
\begin{itemize}
    \item L'utente autenticato si puó eseguire diverse azioni dal modulo dashboard
\end{itemize}
...
\begin{itemize}
    \item L'utente ha effettuato il login ed è autenticato.
    \item Esiste un profilo associato all'utente nel Sistema
    \item L'utente é entrato nel modulo di gestione profilo utente dalla dashboard principale
...
\begin{enumerate}
    ...
    \item L'utente puó modificare uno o più campi del profilo 
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item UC-0D.9 - Modifica informazioni profilo utente
    \end{itemize}
\end{itemize}
...
\subsubsection{UC-0D.9 – Modifica informazioni profilo utente}\label{sec:uc-0d-9}
...
\begin{itemize}
    ...
    \item L'utente amministratore é entrato nel modulo di gestione ruoli dalla dashboard principale.
\end{itemize}
...
\subsubsection{UC-0E.6 – Modifica ruolo utente registrato}\label{sec:uc-0e-8}
...
\subsubsection{UC-0E.8 – Annulla modifica ruolo utente registrato}\label{sec:uc-0e-6}
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item Lo storico è popolato da almeno un elemento
\end{itemize}
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item L'utente si trova nel modulo AI assistant
    \item L'utente ha compilato i campi necessari alla generazione
\end{itemize}

\textbf{Post-condizioni}
\begin{itemize}
    \item Il contenuto viene generato e l'utente puó modicarlo e decidere come usarlo
\end{itemize}
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item L'utente si trova nel modulo 
...
\textbf{Post-condizioni}
\begin{itemize}
    \item Il contenuto viene reso pubblico sulla piattaforma
\end{itemize}
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item Un file non idoneo é stato caricato
\end{itemize}
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item Il file è stato selezionato e tutti i metadati obbligatori sono stati compilati
\end{itemize}

\textbf{Post-condizioni}
\begin{itemize}
    \item Il file viene trasferito fisicamente sul server, indicizzato nel database documentale e analizzato con AI per ottenere le informazioni volute
\end{itemize}
...
\textbf{Post-condizioni}
\begin{itemize}
    \item L'utente puó visualizzare le informazioni riguardanti gli elementi concordanti con il filtro
\end{itemize}
...
\textbf{Post-condizioni}
\begin{itemize}
    \item L'utente puó apportare le modifiche desiderate al documento e puó procedere con le azioni successive 
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item Si é nella pagina lista documenti (UC-2B)
\end{itemize}
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item L'utente si trova nella pagina destinatario (UC-2D) o di apertura del modulo (UC-2A)
\end{itemize}

\textbf{Post-condizioni}
\begin{itemize}
    \item L'utente visualizza tutti i documenti analizzati dall'inizio dell'utilizzo dell'applicativo
\end{itemize}
...
\textbf{Post-condizioni}
\begin{itemize}
    \item Il sistema segno il documento nella lista dei documenti da inviare
\end{itemize}
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item Ci si trova nella pagina di storico dei documenti analizzati
...
\endlastfoot
    ...
    \hline
    RF-026 & L'utente deve avere la possibilità di modificare le informazioni nel proprio profilo utente & \hyperref[sec:uc-0d-9]{UC-0D.9} & Opzionale \\
    \hline
    ...
    \hline
    RF-033 & Un utente con titolo di admin deve poter annullare la modifica al ruolo utente registrato & \hyperref[sec:uc-0e-6]{UC-0E.8} & Opzionale \\
    \hline
    ...
    \hline
    RF-035 & Un utente con titolo di admin deve poter modificare il ruolo di utente registrato & \hyperref[sec:uc-0e-8]{UC-0E.6} & Opzionale \\
    \hline
