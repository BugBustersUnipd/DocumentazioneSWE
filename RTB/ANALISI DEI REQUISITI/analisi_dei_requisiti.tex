...
\begin{figure}[H]
    ...
    \caption{Diagramma del caso d'uso UC-1A – Creazione di un nuovo contenuto tramite prompt}
\end{figure}
...
\textbf{Relazioni con altri casi d'uso (\textit{include} / \textit{extend})}
\begin{itemize}
    \item \textit{include}: UC-1D – Salvataggio e gestione dei prompt (il prompt utilizzato viene sempre salvato nello storico).
    \item \textit{extend}: Nessuna.
\end{itemize}
...
\begin{itemize}
    ...
    \item Il prompt utilizzato è stato salvato nello storico (vedi UC-1D).
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Diagramma del caso d'uso UC-1B – Modifica del contenuto generato}
\end{figure}
...
\textbf{Relazioni con altri casi d'uso (\textit{include} / \textit{extend})}
\begin{itemize}
    \item \textit{include}: UC-1A – Creazione di un nuovo contenuto tramite prompt (è necessario che esista un contenuto generato da modificare).
    \item \textit{extend}: UC-1F – Rigenerazione del contenuto (la modifica può essere arricchita da rigenerazioni automatiche di parti del testo).
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Diagramma del caso d'uso UC-1C – Pubblicazione o scarto del contenuto}
\end{figure}
...
\begin{itemize}
    \item \textit{include}: 
    \begin{itemize}
        \item UC-1A – Creazione di un nuovo contenuto tramite prompt (deve esistere una bozza).
    \end{itemize}
    \item \textit{extend}: Nessuna.
\end{itemize}
...
\textbf{Relazioni con altri casi d'uso (\textit{include} / \textit{extend})}
\begin{itemize}
    \item \textit{include}: UC-1A – Creazione di un nuovo contenuto tramite prompt (i prompt derivano da richieste di generazione).
    \item \textit{extend}: UC-1A – Creazione di un nuovo contenuto tramite prompt (variante in cui il contenuto viene generato a partire da un prompt già presente nello storico anziché da un nuovo prompt).
\end{itemize}

\textbf{Pre-condizioni}
\begin{itemize}
    \item L’utente ha generato almeno un contenuto tramite UC-1A, oppure esistono già prompt salvati in archivio.
\end{itemize}
...
\begin{itemize}
    ...
    \item L’utente può utilizzare i prompt salvati come base per nuove generazioni.
\end{itemize}

\begin{figure}[H]
    \centering
    %\includegraphics[width=1\textwidth]{Diagrammi casi d'uso/UC1D.jpg}
    \caption{Diagramma del caso d'uso UC-1D – Salvataggio e gestione dei prompt}
\end{figure}
...
\textbf{Relazioni con altri casi d'uso (\textit{include} / \textit{extend})}
\begin{itemize}
    \item \textit{include}: UC-1A – Creazione di un nuovo contenuto tramite prompt (deve esistere un contenuto visualizzabile).
    \item \textit{extend}: 
    \begin{itemize}
        \item UC-1A – Creazione di un nuovo contenuto tramite prompt (valutazione immediata dopo la generazione).
        \item UC-1C – Pubblicazione o scarto del contenuto (valutazione dopo la pubblicazione).
    \end{itemize}
\end{itemize}

\textbf{Pre-condizioni}
\begin{itemize}
    \item L’utente ha generato un contenuto ed è in grado di visualizzarlo (anteprima o stato pubblicato).
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Diagramma del caso d'uso UC-1E – Valutazione della qualità del contenuto (Rating)}
\end{figure}
...
\textbf{Relazioni con altri casi d'uso (\textit{include} / \textit{extend})}
\begin{itemize}
    \item \textit{include}: UC-1A – Creazione di un nuovo contenuto tramite prompt (deve esistere una prima versione generata).
    \item \textit{extend}: 
    \begin{itemize}
        \item UC-1A – Creazione di un nuovo contenuto tramite prompt (variante in cui l’utente non accetta la prima generazione e richiede una nuova versione).
        \item UC-1B – Modifica del contenuto generato (variante in cui parte delle modifiche viene ottenuta tramite nuova generazione AI).
    \end{itemize}
\end{itemize}
...
\textbf{Scenario principale}
\begin{enumerate}
    \item L’utente visualizza l’anteprima del contenuto generato.
    \item L’utente decide di volere un’alternativa al contenuto proposto.
    \item Il Sistema invia una nuova richiesta al motore AI (utilizzando lo stesso prompt o una sua variante).
    \item Il motore AI genera una nuova versione del contenuto.
    \item Il Sistema mostra in anteprima la nuova versione, sostituendo o affiancando la precedente, a seconda delle impostazioni.
\end{enumerate}

\textbf{Post-condizioni}
\begin{itemize}
    \item È disponibile una nuova versione del contenuto, che può essere modificata, valutata o pubblicata come negli altri casi d'uso.
\end{itemize}

\begin{figure}[H]
    \centering
    %\includegraphics[width=1\textwidth]{Diagrammi casi d'uso/UC1F.jpg}
    \caption{Diagramma del caso d'uso UC-1F – Rigenerazione del contenuto}
\end{figure}
...
\begin{itemize}
    \item \textit{include}: 
    \begin{itemize}
        \item UC-1A – Creazione di un nuovo contenuto tramite prompt.
        \item UC-1B – Modifica del contenuto generato (se il contenuto è stato modificato).
    \end{itemize}
    \item \textit{extend}: UC-1C – Pubblicazione o scarto del contenuto (la revisione si inserisce come passo alternativo prima della decisione finale).
\end{itemize}
...
\textbf{Post-condizioni}
\begin{itemize}
    \item Il contenuto è pronto per essere gestito tramite UC-1C (pubblicato o scartato).
\end{itemize}

\begin{figure}[H]
    \centering
    %\includegraphics[width=1\textwidth]{Diagrammi casi d'uso/UC1G.jpg}
    \caption{Diagramma del caso d'uso UC-1G – Revisione finale del contenuto}
\end{figure}
...
\textbf{Relazioni con altri casi d'uso (\textit{include} / \textit{extend})}
\begin{itemize}
    \item \textit{include}: nessuna (usa i dati storici già presenti).
    \item \textit{extend}: 
    \begin{itemize}
        \item UC-1A – Creazione di un nuovo contenuto tramite prompt (analisi delle generazioni effettuate).
        \item UC-1C – Pubblicazione o scarto del contenuto (analisi dei tassi di pubblicazione/scarto).
        \item UC-1E – Valutazione della qualità del contenuto (analisi dei rating e della qualità percepita).
    \end{itemize}
\end{itemize}
...
\begin{figure}[H]
    \centering
    %\includegraphics[width=1\textwidth]{Diagrammi casi d'uso/UC1H.jpg}
    \caption{Diagramma del caso d'uso UC-1H – Analisi e reportistica sull’utilizzo}
\end{figure}
...
\textbf{Varianti/Exceptions}
\begin{itemize}
    \item File duplicato $\rightarrow$ UC-2A.1 
    \item Formato non valido $\rightarrow$ UC-2A.2
\end{itemize}

\subsubsection{UC-2A.1 – Inserimento file duplicato}

\subsubsection{UC-2A.2 – Inserimento file in formato non valido}
...
\textbf{Varianti/Exceptions}
\begin{itemize}
    \item Bassa confidenza $\rightarrow$ UC-2B.1 
    \item OCR parziale o fallito $\rightarrow$ UC-2B.2
\end{itemize}

\subsubsection{UC-2B.1 – Bassa confidenza}

\subsubsection{UC-2B.2 – OCR parziale o fallito}
...
\textbf{Varianti/Exceptions}
\begin{itemize}
    \item Multimatch (omonomie) $\rightarrow$ UC-2C.1 
    \item Destinatario mancante $\rightarrow$ UC-2C.2
\end{itemize}

\subsubsection{UC-2C.1 – Multimatch (omonomie)}

\subsubsection{UC-2C.2 – Destinatario mancante}
...
\textbf{Varianti/Exceptions}
\begin{itemize}
    \item Pagine non rilevabili $\rightarrow$ UC-2D.1 
    \item Split incompleto $\rightarrow$ UC-2D.2
\end{itemize}

\subsubsection{UC-2D.1 – Pagine non rilevabili}

\subsubsection{UC-2D.2 – Split incompleto}
...
\textbf{Varianti/Exceptions}
\begin{itemize}
    \item Correzione in bulk $\rightarrow$ UC-2E.1 
\end{itemize}

\subsubsection{UC-2E.1 – Correzione in bulk}


\vspace{0.5cm}

\subsubsection{UC-2F – Creazione di liste di distribuzione}
...
\textbf{Varianti/Exceptions}
\begin{itemize}
    \item Canale non disponibile per alcuni destinatari $\rightarrow$ UC-2F.1 
\end{itemize}

\subsubsection{UC-2F.1 – Canale non disponibile per alcuni destinatari}

\vspace{0.5cm}

\subsubsection{UC-2G – Invio, tracciamento e prova di consegna}

\textbf{Attori}
\begin{itemize}
    \item Operatore di studio CdL.
    \item Sistema NEXUM.
    \item Destinatario del documento.
\end{itemize}

\textbf{Pre-condizioni}
\begin{itemize}
    \item É stata giá generata una lista di distribuzione/dispaccio.
\end{itemize}

\textbf{Scenario principale}
\begin{enumerate}
    \item L'operatore avvia invio (immediato o pianificato)
    \item Il sistema invia su canali selezionati, applica firma digitale/marca temporale se previsto
    \item Registra esiti (consegnato, non consegnato, in attesa, letto) e genera ricevute
    \item Il destinatario riceve la notifica e accede al documento (portale/app)
\end{enumerate}

\textbf{Post-condizioni}
\begin{itemize}
    \item Tracciamento completo in registro di dispaccio; KPI aggiornati.
\end{itemize}

\textbf{Varianti/Exceptions}
\begin{itemize}
    \item Bounce o mancata consegna $\rightarrow$ UC-2G.1 
    \item Revoca/annullo prima del download $\rightarrow$ UC-2G.2
\end{itemize}

\subsubsection{UC-2G.1 – Bounce o mancata consegna}

\subsubsection{UC-2G.2 – Revoca/annullo prima del download}

\vspace{0.5cm}


\subsubsection{UC-2H – Consultazione storico documentale e audit locale}
...
\textbf{Varianti/Exceptions}
\begin{itemize}
    \item Richiesta di oblìo (GDPR) $\rightarrow$ UC-2H.1 
\end{itemize}

\subsubsection{UC-2H.1 – Richiesta di oblìo (GDPR)}

\vspace{0.5cm}


\subsubsection{UC-2I – Gestione ruoli, permessi e policy (Sicurezza & GDPR)}
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item Tenant e ruoli configurati.
\end{itemize}

\textbf{Scenario principale}
\begin{enumerate}
    \item Definizione ruoli (Admin CdL, Editor, Viewer, Auditor).
    \item Policy retention per tipologia documentale (es. 10 anni per cedolini).
    \item Configurazione consensi e informative per destinatari.
\end{enumerate}

\textbf{Post-condizioni}
\begin{itemize}
    \item Accessi e trattamenti compliant (principi di minimizzazione e tracciabilità).
\end{itemize}

\textbf{Varianti/Exceptions}
\begin{itemize}
    \item Accesso non autorizzato $\rightarrow$ UC-2I.1 
\end{itemize}

\subsubsection{UC-2H.1 – Accesso non autorizzato}

\subsubsection{UC-2J – Monitoraggio qualità AI e miglioramento continuo}
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item Pipeline AI in produzione con raccolta metriche.
\end{itemize}

\textbf{Scenario principale}
\begin{enumerate}
    \item Raccolta metriche: confidenza media, percentuale interventi manuali, accuratezza mapping, tempi medi.
    \item Analisi rating (feedback operatori) per addestrare regole/LLM e aggiornare soglie.
    \item Rilascio miglioramenti (nuovi pattern, dizionari cliente, layout engine).
\end{enumerate}

\textbf{Post-condizioni}
\begin{itemize}
    \item Modelli e regole ottimizzati; riduzione errori e tempi di ciclo.
\end{itemize}
