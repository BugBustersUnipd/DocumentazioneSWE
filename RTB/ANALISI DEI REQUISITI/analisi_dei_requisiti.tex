...
\newcommand{\CurrentVersion}{0.1.7} % ultima versione, da cambiare ad ogni push significativo
...
\rowcolor{secondaryblue!10} \CurrentVersion & 23/12/2025 & Correzioni minime nel contenuto e nel registro delle modifiche. & Linor Sadè & - & - \\
\hline
\rowcolor{secondaryblue!10} 0.1.6 & 25/12/2025 & - & Leonardo Salviato & - & - \\
\hline
\rowcolor{secondaryblue!10} 0.1.5 & 24/12/2025 & - & Leonardo Salviato & - & - \\
\hline
\rowcolor{secondaryblue!10} 0.1.4 & 23/12/2025 & Correzioni e aggiunte minime alla sezione 1. Migliorie grafiche & Linor Sadè & - & - \\
\hline
\rowcolor{secondaryblue!10} 0.1.3 & 12/12/2025 & Aggiunta sotto-casi d'uso alla sezione 2 & Leonardo Salviato & - & - \\
\hline
...
\begin{itemize}
    ...
    \item \textbf{Gestione risultato}:
    possibilità di eseguire interventi manuali sui risultati delle generazioni
...
\begin{itemize}
    ...
    \item \textbf{Redattore}: Un qualsiasi dipendente incaricato con la creazioni di un post
    ...
    \item \textbf{Amministratore}: Gestisce la configurazione tecnica dell'applicazione standalone. Si occupa della creazione degli utenti, della gestione dei ruoli.
...
\textbf{Post-condizioni}
\begin{itemize}
    \item L'utente ha fatto generare un contenuto pubblicabile dal sistema
    \item Il contenuto è stato salvato nello storico
\end{itemize}
...
\begin{enumerate}
    ...
    \item Il sistema genera il contenuto e lo salva nello storico
\end{enumerate}
...
\begin{enumerate}
    ...
    \item L'utente può ottenere informazioni specifiche su risultati passati
\end{enumerate}
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item L'utente ha generato un contenuto tramite UC-1A
\end{itemize}

\textbf{Post-condizioni}
\begin{itemize}
    \item L'utente ha la possibilità di modificare la generazione
\end{itemize}


\textbf{Scenario principale}
\begin{enumerate}
    \item L'utente visualizza il contenuto generato
    \item L'utente avvia la modifica del contenuto generato
\end{enumerate}
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item L'utente avvia la modifica del contenuto (UC-1D)
\end{itemize}
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item L'utente ha avviato la modifica del contenuto (UC-1D)
\end{itemize}
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item L'utente ha avviato la sezione dedicata allo storico (UC-1B)
    \item Lo storico ha almeno un elemento
\end{itemize}
...
\textbf{Scenario principale}
\begin{enumerate}
    \item L'utente inserisce una parola chiave (es. parte del prompt o data) nel campo di ricerca
...
\textbf{Scenario principale}
\begin{enumerate}
    \item L'utente genera un contenuto tramite AI
...
\begin{enumerate}
    ...
    \item Il sistema assegna automaticamente la valutazione minima al contenuto
    \item Il sistema pulisce l'area di anteprima
    \item Il sistema riavvia il flusso di generazione contenuto
\end{enumerate}
\textbf{Relazioni con altri casi d'uso (\textit{include} / \textit{extend})}
\begin{itemize}
    \item \textit{include}: 
    \begin{itemize}
        \item UC-1A – Generazione contenuto tramite AI
    \end{itemize}
\end{itemize}
...
\textbf{Post-condizioni}
\begin{itemize}
    \item L'utente ha fatto analizzare al sistema il documento o l'insieme di documenti caricati
\end{itemize}
...
\begin{enumerate}
    ...
    \item L'utente carica il file da analizzare e ha la possibilità di compilare i campi
    \item Il sistema analizza il file fornito
\end{enumerate}
...
\begin{enumerate}
    ...
    \item L'utente ha caricato un file che poi è stato analizzato dal sistema
...
\textbf{Pre-condizioni}
\begin{itemize}
    \item Si è selezionato un documento da una delle liste disponibili
\end{itemize}
...
\textbf{Scenario principale}
\begin{enumerate}
    \item Un utente ha selezionato uno o piú documenti da inviare a uno o piú destinatari e prepara in messaggio da inviare assieme ai documenti.
\end{enumerate}
...
\endlastfoot
    ...
    \hline
    RF-035 & Un utente con titolo di admin deve poter modificare il ruolo di utente registrato & \hyperref[sec:uc-0e-6]{UC-0E.6} & Opzionale \\
    \hline
    ...
    RF-038 & Un utente deve avere la possibilità di generare contenuti AI & \hyperref[sec:uc-1a]{UC-1A} & Obbligatorio \\
    \hline
    ...
    \hline
    RF-052 & C'è la possibilità che il prompt non abbia neanche un elemento data la mancanza di generazioni & \hyperref[sec:uc-1b-1]{UC-1B.1} & Obbligatorio \\
    \hline
    ...
    \hline
    RF-058 & Un utente deve avere la possibilità di rigenerare un contenuto tramite AI & \hyperref[sec:uc-1h]{UC-1H} & Obbligatorio \\
    \hline
    RF-059 & Deve avvenire il salvataggio di ogni generazione nello storico & \hyperref[sec:uc-1c-3]{UC-1C.3} & Obbligatorio \\
    \hline
    RF-060 & Deve avvenire una valutazione di ogni contenuto generato & \hyperref[sec:uc-1i]{UC-1i} & Obbligatorio \\
    \hline
    RF-061 & Un utente deve avere la possibilità di scartare un contenuto generato & \hyperref[sec:uc-1j]{UC-1J} & Obbligatorio \\
    \hline
    RF-062 & Un utente deve avere la possibilità di pubblicare un contenuto generato & \hyperref[sec:uc-1k]{UC-1K} & Obbligatorio \\
    \hline
    ...
    \hline
    RF-090 & Deve esserci un controllo per "Nessun documento riconosciuto" & \hyperref[sec:uc-2b-1]{UC-2B.1} & Obbligatorio \\
    \hline
    ...
    \hline
    RF-099 & Un utente deve avere la possibilità di visualizzare la lista informazioni destinatari & \hyperref[sec:uc-2d]{UC-2D} & Obbligatorio \\
    \hline
    RF-100 & Un utente deve avere la possibilità di visualizzare un elemento dalla lista informazioni destinatari & \hyperref[sec:uc-2d-1]{UC-2D.1} & Obbligatorio \\
    \hline
    ...
    \hline
    RF-104 & Un utente deve avere la possibilità di visualizzare il codice fiscale di un destinatario & \hyperref[sec:uc-2d-2]{UC-2D.2} & Obbligatorio \\
    \hline
    ...
    \hline
    RF-109 & C'è la possibilitá che non siano mai stati registrati documenti e che quindi l'applicativo agisca di conseguenza & \hyperref[sec:uc-2e-2]{UC-2E.1} & Obbligatorio \\
    \hline
    ...
    \hline
    RF-127 & Il sistema non deve mostrare niente se nessun template è stato salvato all'interno dello storico & \hyperref[sec:uc-2g-2]{UC-2G.2} & Obbligatorio \\
    \hline
...
\vspace{0.5cm}
\begin{longtable}{|L{\wCod}|L{\wDesc}|L{\wFonti}|L{\wPrio}|}
\caption{Tabella dei Requisiti di Qualità}\label{tab:reqfunzionali}\\
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Fonti} & \textbf{Priorità} \\
\hline
\endfirsthead
...
\vspace{0.5cm}
\begin{longtable}{|L{\wCod}|L{\wDesc}|L{\wFonti}|L{\wPrio}|}
\caption{Tabella dei Requisiti di Vincolo}\label{tab:reqfunzionali}\\
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Fonti} & \textbf{Priorità} \\
\hline
\endfirsthead
...
\vspace{0.5cm}
\begin{longtable}{|L{\wCod}|L{\wDesc}|L{\wFonti}|L{\wPrio}|}
\caption{Tabella dei Requisiti Prestazionali}\label{tab:reqfunzionali}\\
\hline
\textbf{Codice} & \textbf{Descrizione} & \textbf{Fonti} & \textbf{Priorità} \\
\hline
\endfirsthead
