...
\hline
\rowcolor{secondaryblue!10} 2.0.0 & 01/03/2025 & Approvato per PB & - & - & Marco Favero \\
\hline
\rowcolor{secondaryblue!10} 1.4.0 & 28/02/2025 & Verificata sezione Formazione & - & Alberto Pignat & - \\
\hline
...
\begin{itemize}
    ...
    \item {Verbali}
    \item \href{https://bugbustersunipd.github.io/DocumentazioneSWE/CANDIDATURA/CANDIDATURA%20PROGETTO/Candidatura%20progetto.pdf}{Lettera di candidatura}
...
La Specifica Tecnica documenta l'architettura di sistema e le scelte progettuali adottate dal gruppo BugBusters per lo sviluppo del prodotto software. Esso definisce nel dettaglio i design pattern scelti, lo stack tecnologico impiegato, l'architettura ad alto e basso livello, e il tracciamento tra i componenti software e i requisiti. Il documento funge da linea guida operativa per i programmatori, con l'obiettivo specifico di garantire una solida base strutturale che favorisca la scalabilità, la manutenibilità del codice e la corretta implementazione delle funzionalità.
...
l'organizzazione del lavoro e la successiva implementazione del codice.\\
...
Attraverso questa sperimentazione iniziale è possibile individuare gli strumenti più adatti e, in collaborazione con la proponente, 
delineare le funzionalità essenziali che costituiranno la prima versione del prodotto, ovvero il Minimum Viable Product.


\subsubsubsection{Qualità dell'architettura}
Per una buona architettura software si richiedono le seguenti qualità:
\begin{itemize}
    \item \textbf{Sufficienza}: L'architettura assicura la piena soddisfazione dei requisiti funzionali e non funzionali stabiliti per il sistema.
    \item \textbf{Comprensibilità}: La struttura architetturale è chiara e facilmente interpretabile da tutti gli stakeholder, agevolando manutenzione ed evoluzione.
    \item \textbf{Modularità}: Il sistema è suddiviso in moduli autonomi e ben definiti, ciascuno sviluppabile, testabile e aggiornabile indipendentemente.
    ...
    \item \textbf{Riusabilità}: Componenti e moduli sono riutilizzabili in contesti differenti, riducendo duplicazioni e aumentando l'efficienza.
    \item \textbf{Efficienza}: L'architettura ottimizza l'uso delle risorse per garantire prestazioni elevate con consumo contenuto.
    \item \textbf{Affidabilità}: Il software opera in modo coerente e prevedibile nel tempo, rispettando le funzionalità attese.
    ...
    \item \textbf{Sicurezza rispetto a malfunzionamenti}: L'architettura contiene meccanismi che permettono al sistema di continuare a funzionare anche in caso di guasti.
    \item \textbf{Sicurezza rispetto a intrusioni}: Sono previste protezioni contro accessi non autorizzati, garantendo riservatezza e integrità dei dati.
    ...
    \item \textbf{Coesione}: Ogni modulo presenta componenti che collaborano per uno scopo comune, evitando responsabilità frammentate.
    \item \textbf{Basso accoppiamento}: Le dipendenze tra moduli sono minimizzate per migliorare manutenibilità e capacità di evolvere il sistema.
\end{itemize}
...
Inoltre, i diagrammi supportano la fase di sviluppo, fungendo da riferimento per l'implementazione e la manutenzione del codice.\\
...
modo efficace la struttura del sistema sia a stakeholder tecnici che non tecnici. In particolare, vengono utilizzati diagrammi di 
...
L'utilizzo di questi schemi progettuali contribuisce a migliorare la qualità del codice, rendendolo più modulare, riutilizzabile e 
facilmente estendibile. La scelta dei pattern avviene in base alle esigenze specifiche del progetto, con l'obiettivo di garantire 
...
linguaggi e le tecnologie scelte. L'obiettivo principale è realizzare un prodotto funzionante che rispetti 
i requisiti stabiliti, mantenendo al contempo chiarezza, coerenza e qualità del codice. Una buona fase di 
codifica contribuisce a ridurre errori, semplificare le fasi successive di test e manutenzione, e 
garantire una maggiore affidabilità del sistema.
...
necessario.\\ 
Ulteriori accorgimenti includono l'uso di sistemi di versionamento per tracciare le modifiche, la 
revisione del codice tra pari (code review) e l'esecuzione di test durante lo sviluppo. \\
Queste pratiche permettono di individuare tempestivamente eventuali problemi e di migliorare 
progressivamente la qualità complessiva del prodotto.
...
cambiamenti subiti da ciascun elemento sviluppato;
    \item \textbf{Valutazione della configurazione}: come determinare l'efficacia del prodotto
sviluppato, ossia la sua conformità ai requisiti;

\end{itemize}

\subsubsection{Identificazione della configurazione}
Data la complessità architetturale del progetto definito nel capitolato\G{}, l'identificazione della configurazione risulta un'attività fondamentale. In fase di progettazione, le varie parti del sistema verranno pertanto schematizzate e documentate in un artefatto specifico, da redigere e approvare durante la Product Baseline\G{} (PB).
...
La valutazione della configurazione verifica il perfetto allineamento del software ai requisiti e al design architetturale, garantendo tramite il Tracciamento dei Requisiti il rilascio di un prodotto efficiente (pienamente conforme alle specifiche) e sufficiente (privo di complessità superflue). Per assicurare tale aderenza durante l'intero ciclo di sviluppo, il gruppo adotterà una strategia integrata: il codice verrà esplicitamente commentato per tracciare il legame diretto con il requisito soddisfatto, mentre il testing automatizzato e le metriche di code coverage permetteranno di validare costantemente le funzionalità e individuare tempestivamente eventuali aree scoperte. A supporto di ciò, la rigorosa modularità del sistema separerà le responsabilità architetturali, favorendo una tracciabilità netta e lineare tra ogni singolo componente e i requisiti di riferimento.



\subsection{Accertamento della Qualità}
Il processo di Accertamento della Qualità (o \textit{Quality Assurance}) ha lo scopo di garantire che le metodologie adottate e i prodotti realizzati dal gruppo BugBusters soddisfino i requisiti minimi di efficienza\G{} ed efficacia prestabiliti.
...
L'individuazione e la gestione dei rischi di progetto si articola in tre diverse fasi descritte di seguito:

\begin{itemize}
    \item \textbf{Identificazione}: al fine di prevenire problemi futuri si vanno ad individuare le parti critiche del progetto analizzando ogni potenziale rischio associato. Per ogni rischio individuato occorre specificare il nome, una descrizione del rischio, il livello di probabilità e l'impatto. Le tipologie di rischio e le relative tabelle di dettaglio sono descritte nel \textit{Piano di Progetto}. Ad ogni rischio è inoltre associato un codice univoco con il seguente formato:
    ...
    \begin{itemize}
        \item \textbf{G}: Rischio Globale\G{};
        \item \textbf{T}: Rischio Tecnologico\G{};
        \item \textbf{I}: Rischio Individuale\G{}.
    \end{itemize}
    L'ID è un valore numerico incrementale a una cifra.
    
    \item \textbf{Mitigazione}: per ogni rischio individuato si definiscono a priori le possibili contromisure da attuare nel caso in cui il problema si presenti, al fine di ridurne l'impatto sul progetto.
    
    \item \textbf{Monitoraggio}: i rischi individuati vengono costantemente monitorati dal Responsabile\G{} durante l'intero ciclo di vita del progetto, al fine di individuare tempestivamente la loro occorrenza e applicare le relative contromisure stabilite.
\end{itemize}
...
vita del software. La finalità principale è assicurare che il prodotto risponda pienamente ai requisiti e alle aspettative,
garantendo allo stesso tempo alti livelli di qualità ed efficienza. \\
...
orientati al continuo perfezionamento del prodotto.

\subsubsection{Ciclo di miglioramento continuo}
Il processo di miglioramento si articola in quattro fasi principali. 
La prima consiste nell'individuazione dei processi organizzativi da adottare nelle varie attività di progetto lungo tutto il 
...
costante. \\
La terza fase è dedicata alla verifica dei processi, effettuata sulla base degli obiettivi stabiliti, delle metriche definite e
...
favorendo un'evoluzione continua dei processi e riducendo il rischio di regressioni. 
Questo approccio iterativo consente di mantenere nel tempo elevati livelli di qualità ed efficienza.\\
...
affrontare in modo efficace e consapevole le possibili criticità che possono emergere durante lo sviluppo del progetto.\\
...
adottate.\\
La formazione rappresenta quindi un elemento essenziale per la buona riuscita del progetto, contribuendo non solo alla qualità del 
...
Nel corso dello sviluppo del progetto, ciascun componente del gruppo è chiamato a sviluppare in modo autonomo le competenze necessarie 
...
raggiungimento degli obiettivi prefissati e favorendo al tempo stesso un avanzamento più efficace e consapevole dell'intero progetto.
