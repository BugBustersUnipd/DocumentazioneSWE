...
TS-Q-006 & Verifica\G che ci sia il manuale utente. & RQ-06 & NI \\ \hline
...
Sidekiq e PWA\G con Next.js. & RV-06 & NI \\ 
\hline
TS-V-007 & Verifica\G che i requisiti minimi di sistema siano rispettati: RAM 2GB,
Spazio 20 GB SSD, CPU 1-2 vCPU. & RV-07 & NI \\ \hline
TS-V-008 & Verifica\G che i requisiti minimi dell'utente siano rispettati: Desktop 4GB,
Mobile 2GB, Rete 3G stabile. & RV-08 & NI \\ \hline
TS-V-009 & Verifica\G che i browser supportati siano Chrome, Safari, Edge e Firefox. & RV-09 & NI \\ \hline
...
\subsubsubsection{PB}
Durante la fase di Product Baseline (PB), il Valore Guadagnato (EV)\G e il Valore Pianificato (PV)\G hanno mantenuto un allineamento pressoché perfetto, convergendo al traguardo finale nello Sprint 12. Ciò attesta l'assenza di deviazioni di scope e il rigoroso rispetto del piano di sviluppo preventivato nella precedente baseline.
\newpage

\newpage

\subsection{MPC18 - Nuova metrica 4}
\subsubsubsection{RTB}
...
\subsubsubsection{PB}
Nel periodo di PB, la curva dell'Actual Cost (AC)\G ha mantenuto un tasso di crescita lineare e predicibile, speculare al progressivo azzeramento dell'Estimate to Complete (ETC)\G. Il raggiungimento di un ETC\G pari a zero allo Sprint 12, unito a un EAC stabile, dimostra una corretta consuntivazione delle risorse, senza sforamenti di budget tardivi durante l'implementazione finale.
\newpage
...
\subsection{MPD14 - Cyclomatic Complexity}
\begin{figure}[H]
  \centering
  \IfFileExists{grafici/MPD14.png}{%
    \includegraphics[width=0.9\textwidth]{grafici/MPD14.png}
  }{%
    \fbox{\parbox[c][5cm][c]{0.85\textwidth}{\centering Grafico non trovato: grafici/MPD14.png}}
  }
  \caption{Grafico per periodo di MPD14}
\end{figure}
\subsubsubsection{RTB}
Non sono presenti misurazioni.
\subsubsubsection{PB}
...
In questa sezione sono riportate le metriche misurate puntualmente, per le quali non e stato associato un grafico temporale. Si tratta di indicatori con valore statico, binario o calcolato su base aggregata, per cui la rappresentazione tabellare risulta piu chiara e immediata.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{1.8cm}|X|>{\raggedright\arraybackslash}p{3cm}|>{\raggedright\arraybackslash}p{3cm}|}
    ...
    \hline
    MPD11 & Click Count (Funzioni principali) & $\leq 5$ click & 7.5 click \\
    \hline
    MPD12 & User Error Rate (Errori validazione\G) & $\leq 10\%$ & 9.46\% \\
    \hline
    MPD13 & Blocker Code Smells & 0 & 4 \\
    \hline
    MPD15 & Comment Intensity & $\geq 10\%$ & 6.66\% \\
    \hline
